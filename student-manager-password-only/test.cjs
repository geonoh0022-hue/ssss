...
async function login(){const r=await call('/api/login','POST',{password:'a'});assert.equal(r.status,200);assert.match(r.headers.get('set-cookie'),/HttpOnly/);return r.headers.get('set-cookie').split(';')[0];}
try{await start();assert.equal((await call('/api/data')).status,401);assert.equal((await call('/api/login','POST',{password:'wrong'})).status,401);
...
for(let i=0;i<10;i++)assert.equal((await call('/api/login','POST',{password:'wrong'})).status,401);
